...
def triangle(one, two, three)
  ...
  if one == two && two == three && three == one then :equilateral
  elsif arr.min == (arr.max.to_f / 2) then :isosceles
  elsif (one != two && two != three && three != one) && arr.min + arr[1] > arr[-1] then :scalene
  else
    :invalid
  end
end
